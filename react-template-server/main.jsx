...
<body>
    <!-- comment out above markup when go server embeds this into a golang html template -->
    ...
    <!-- comment out below markup when go server embeds this into a golang html template -->
</body>
</html>`;
...
app.post('/template/:name', async (c) => {
    try {
        ...
        return c.html(html(renderToString(<App {...props} />), clientScript(name, props)));
    } catch (err) {
...
serve({ fetch: app.fetch, port: 3001 }, (info) => console.log("Start listening on port", info.port));